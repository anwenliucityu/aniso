...
class DifferentialDisplacement():
    ...
    def plot(self, system, component, ddmax, plotxaxis='x', plotyaxis='y',
             ...
             matplotlib_axes=None, cmap='bwr', xbins=200, ybins=200, 
             scale=1, nye_index=[0,0],b_length=4.05,calculation=True, disl_line_pos = False):
        ...
        # ave position
        dxdy = 1/x.shape[0] ## have to be changed when change material or orientation
        x0 = np.sum(x*v*dxdy)
        y0 = np.sum(y*v*dxdy)
        frac = np.sum(v*dxdy)
        x0 = x0/frac
        ...
        xlim = [x0+xlim[0], x0+xlim[1]]
        ylim = [y0+ylim[0], y0+ylim[1]]
        ...
        if calculation==True:
            ...
            x0 = np.sum(x*v*dxdy)
            y0 = np.sum(y*v*dxdy)
            x_sq = np.sum(x**2*v*dxdy)
            y_sq = np.sum(y**2*v*dxdy)
            xy = np.sum(x*y*v*dxdy)
            frac = np.sum(v*dxdy) 
            x0 = x0/frac
            y0 = y0/frac
        ...
        arrowlengths, arrowcenters = self.__buildarrows(T, plotbox, component, ddmax, b_length)
    ...
    def __buildarrows(self, T, plotbox, component, ddmax, b_length):
        ...
        if isinstance(component, str) and component == 'projection':
            ...
            ddcomponents[ddcomponents<0.1*b_length] = 0
